...
function addExpense() {
    ...
    newRow.innerHTML = `
        <td class="py-2 px-4">${expenseName}</td>
        <td class="py-2 px-4">${description}</td>
        <td class="py-2 px-4">PKR ${amount}</td>
        <td class="py-2 px-4">${paymentMethod}</td>
        <td class="py-2 px-4">${date}</td>
        <td class="py-2 px-4 no-pdf">
            <button onclick="editExpense(this)" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-2 rounded cursor-pointer">Edit</button>
            <button onclick="deleteExpense(this)" class="bg-red-500 hover:bg-red-600 text-white font-bold py-1 px-2 rounded cursor-pointer">Delete</button>
        </td>
    `;
...
// Function to load expenses from localStorage when the page is loaded
function loadExpenses() {
    ...
    expenses.forEach(function(expense) {
        var newRow = table.insertRow();
        newRow.innerHTML = `
            <td class="py-2 px-4">${expense.expenseName}</td>
            <td class="py-2 px-4">${expense.description}</td>
            <td class="py-2 px-4">PKR ${expense.amount}</td>
            <td class="py-2 px-4">${expense.paymentMethod}</td>
...
function downloadData(format) {
    ...
    if (format === 'pdf') {
        // Get all "Actions" cells (header and body)
        var actionHeader = table.querySelector('th:nth-child(6)');
        var actionCells = table.querySelectorAll('td:nth-child(6)');

        // Hide the Actions column by setting style display to 'none'
        ...
        html2pdf()
            ...
            .save('expenses.pdf')
            .then(function() {
                // Restore the visibility of the Actions column after PDF generation
